...
def minifyPy(target):
	...
	'''
	os.system(f'for py in `find {target} -name "*.py"`; do echo "Minifying ${{py}}..."; pyminify --remove-literal-statements ${{py}} > ${{py}}.min; mv ${{py}}.min ${{py}}; done')
...
def main(argv):
	...
	try:
		opts, args = getopt.getopt(argv,"ht:s:m:c:z:n:",["target=","source=","minify=","compile=","zip=","name="])
