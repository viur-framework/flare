...
def formatOneEntry(
    key, format, data, structure=None, prefix=None, language=None, context=None, _rec=0
):
    res = format
    val = data[key]
    ...
    if isinstance(val, dict):  # if bone is multilang, only render current lang
    ...
    ):  # if bone is relationalbone with rel and dest
        if struct and "dest" in val[0] and "rel" in val[0]:
            ...
            res = res.replace(
                ...
                ", ".join(
                    [
                        formatString(format, v, struct, [], language, _rec=_rec + 1)
                        for v in val
                    ]
                ),
            )
